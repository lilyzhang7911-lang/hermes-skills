...
description: >-
  Use when the user wants to iteratively optimize an agent skill through automated evaluation.
  Upload a skill folder (SKILL.md + scripts), generate test scenarios and eval criteria,
  then run a multi-agent optimization loop (Executor scores, Analyst diagnoses failures,
  Mutator applies surgical fixes). Inspired by Karpathy's autoresearch methodology.
license: Apache-2.0
metadata:
...
compatibility: >-
  Requires Google ADK (google-adk) and Gemini API key. Backend uses FastAPI + SSE streaming,
  Frontend uses Next.js 15 + React 19 + Tailwind CSS v4. Runs locally.
---

# Self-Improving Agent Skills

Automatically optimize agent skills using a multi-agent system built with **Google ADK (Agent Development Kit)** and **Gemini**. Upload a skill, let the agents generate test scenarios and evaluation criteria, then watch as three specialized ADK agents collaborate to improve your skill through iterative optimization.
